...
이번 재검수에서는 코드 실행 환경의 학교 도메인 DNS/HTTP 연결이 실패했습니다.
따라서 **이 크롤러로 학교 데이터를 실제 적재하는 검증은 완료하지 못했습니다**.
웹 페이지를 별도 도구로 열람하는 것과 현재 크롤러의 실제 수집 성공은 다릅니다.
이전 페이지 열람 기록으로 실수집 완료를 주장하지 않습니다.

`tests/fixtures`는 합성 HTML이며 학교에서 받은 실제 HTML이라고 표시하지 않았습니다.
CMS 선택자 구현과 HTTP/robots/재시도/중복/변경/실패 보존 흐름을 합성 응답으로 시험했습니다.
학교에서 직접 허용된 테스트 실행 후 세 출처별 목록·상세 HTML에 맞게 선택자를 검수해야 합니다.
