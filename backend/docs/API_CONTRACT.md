# SCNU PICK API 계약 — 구현 제안 v1

이 문서는 현재 백엔드 코드와 일치하는 **팀 검토용 계약**입니다.
공유 저장소 루트 `docs/api-contract.md`의 미합의 상태를 임의로 확정하지 않습니다.
전체 필드·필수 여부는 함께 제공한 `openapi.json` 또는 실행 중인 `/docs`가 기준입니다.
프론트엔드 구현 파일은 포함하지 않았습니다.
...
브라우저 영구 저장 방식은 아직 합의하지 않았습니다.
초기 연결은 메모리 보관으로 검증하며, 새로고침 이후 로그인 유지 방식은 보안 검토 후 정합니다.
